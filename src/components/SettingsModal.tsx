...
import { AccountSettingsSection } from './sync/AccountSettingsSection';
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-lg w-full space-y-5 shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-white">Configurações & Nuvem</h2>
            <p className="text-xs text-slate-400">Preferências e sincronização PC / Celular</p>
          </div>
        </div>
        ...
        <div className="flex rounded-xl bg-slate-950 p-1 border border-slate-800">
          ...
          >
            <Cloud className="w-3.5 h-3.5" /> Conta & Sincronização
          </button>
        </div>
        ...
        {activeTab === 'account' ? (
          ...
          <div className="space-y-4">
            ...
            {onOpenDiagnostic && (
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-2">
                ...
                <p className="text-[11px] text-slate-400">
                  Verifique integridade, imagens ausentes e status de sincronização das coleções.
                </p>
                <button
                  onClick={() => {
                    onClose();
            ...
            <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
              <span className="text-xs font-bold text-slate-300 block">Backup Local Rápido</span>
              <p className="text-[11px] text-slate-400">
                Exporte e importe snapshots manuais em formato JSON.
              </p>
